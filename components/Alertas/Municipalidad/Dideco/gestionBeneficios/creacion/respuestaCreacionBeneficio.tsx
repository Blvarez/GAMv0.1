...
export default function RespuestaCreacionBeneficio({ datoExtra, tipoDato, estadoCreacion, cambioEstadoCreacion, error }: Props) {
    ...
    function erroro(numero: number) {
        if (numero == 1) {
            ...
            cambioTexto = (
                <h1 className="text-center text-[#427DA2]"><strong>Stock tiene que ser mayor a 0 y menor a 999999..!</strong></h1>
            )
            return (
                <div>
                    <button className="absolute ml-[405px] mt-[-20px] text-red-500" onClick={() => { cambioEstadoCreacion(false) }}><FontAwesomeIcon className="text-3xl colorX" icon={faTimes}></FontAwesomeIcon></button>
                    <h1 className="ml-[145px] mt-[15px]"><FontAwesomeIcon icon={faTimesCircle} className="text-[#003352] text-9xl" /></h1>
                    <h1 className="text-center text-2xl text-[#427DA2]"><strong>Stock tiene que ser mayor a 0 y menor a 999999..!</strong></h1>
                </div>
            )
        }
        return null;
